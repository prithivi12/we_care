<!DOCTYPE html>
    ...
    <body>
        ...
        if(isset($_POST['signin'])){
            ...
            $checkuser=$con->connection->query("SELECT * from signup where username='$username' and password='$password'");
            $checkadmin=$con->connection->query("SELECT * from admin where username='$username' and password='$password'");
            if($checkadmin->num_rows>0){
                $_SESSION['admin']=$username;
                ?>
                <script>window.location.assign("admin-dash.php");</script>
                <?php
            }
            else{
